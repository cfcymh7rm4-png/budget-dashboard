...
// ---- plugin:feishu_bitable_import_daily_consumption_data_1 ----
// ============================================================
// 插件 feishu_bitable_import_daily_consumption_data_1 (导入每日消耗数据到飞书多维表格) 的类型定义
// 由 get_plugin_ai_json 自动生成
// ============================================================

export interface FeishuBitableImportDailyConsumptionData1AggregatequeryInput {
  /** 筛选条件,严格参考”筛选条件说明”填写 */
  filter?: {
  conjunction: string;
  conditions: {
...
export interface FeishuBitableImportDailyConsumptionData1AggregatequeryOutput {
  /** 下一页的分页标记 */
  pageToken?: string;
  /** 聚合查询结果,每一个元素都是一个聚合项 */
  result: {
...
export interface FeishuBitableImportDailyConsumptionData1BatchaddrecordsInput {
  /** 要新增的记录列表，最多 500 条 */
  records: {
  record: {
  日期: number;
...
export interface FeishuBitableImportDailyConsumptionData1BatchaddrecordsOutput {
  /** 新增成功的记录列表 */
  records: {
  id: string;
}[];
...
export interface FeishuBitableImportDailyConsumptionData1BatchupdaterecordsInput {
  /** 要更新的记录列表，最多 500 条 */
  records: {
  id: string;
  record: {
...
export interface FeishuBitableImportDailyConsumptionData1BatchupdaterecordsOutput {
  /** 更新成功的记录列表 */
  records: {
  id: string;
}[];
...
export interface FeishuBitableImportDailyConsumptionData1DeleterecordsInput {
  /** 要删除的记录ID列表，最多 500 条 */
  recordIDs: string[];
}

export interface FeishuBitableImportDailyConsumptionData1DeleterecordsOutput {
  /** 是否删除成功 */
  success: boolean;
}

export interface FeishuBitableImportDailyConsumptionData1GetrecordInput {
  /** 记录ID */
  recordID: string;
}

export interface FeishuBitableImportDailyConsumptionData1GetrecordOutput {
  /** 记录ID */
  id: string;
  /** 记录内容,如果记录不存在,则为空 */
  record?: {
...
export interface FeishuBitableImportDailyConsumptionData1SearchrecordsInput {
  /** 分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 pageToken */
  pageToken?: string;
  /** 分页大小，最大值为 500 */
  pageSize?: number;
...
export interface FeishuBitableImportDailyConsumptionData1SearchrecordsOutput {
  /** 下一页的分页标记 */
  pageToken?: string;
  /** 总记录数 */
  total?: number;
...
// ---- end:feishu_bitable_import_daily_consumption_data_1 ----